...
st.title("Registrar Movimentação de Aparelho")
st.markdown("---")
...
def registrar_movimentacao(aparelho_id, colaborador_id, novo_status_id, localizacao, observacoes):
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        conn.rollback()
        st.error(f"Ocorreu um erro ao registrar a movimentação: {e}")
    finally:
        conn.close()
...
with st.form("form_movimentacao", clear_on_submit=True):
    ...
    colaboradores_dict_com_nenhum = {"Nenhum": None}
    colaboradores_dict_com_nenhum.update(colaboradores_dict)
    
    aparelho_selecionado_str = st.selectbox("Selecione o Aparelho*", options=aparelhos_dict.keys())
    colaborador_selecionado_str = st.selectbox("Atribuir ao Colaborador", options=colaboradores_dict_com_nenhum.keys())
    ...
    if st.form_submit_button("Registrar Movimentação"):
        if not aparelho_selecionado_str or not novo_status_str:
            ...
            colaborador_id = colaboradores_dict_com_nenhum[colaborador_selecionado_str]
            novo_status_id = status_dict[novo_status_str]
            registrar_movimentacao(aparelho_id, colaborador_id, novo_status_id, nova_localizacao, observacoes)
